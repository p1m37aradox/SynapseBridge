...
async def mempalace_status():
    ...
    from mempalace.mcp_server import _get_collection, _call_kg, _config
    from mempalace.palace_graph import graph_stats
    
    col = _get_collection()
    stats = _call_kg(lambda kg: graph_stats(kg)) if col else {}
    
    # FIX: Check if count exists on col, otherwise use stats
    if hasattr(col, 'count'):
        drawer_count = col.count()
    else:
        drawer_count = stats.get("total_drawers", stats.get("drawer_count", "Connected"))
...
async def mempalace_search(q: str = "", wing: str = None, limit: int = 5):
    ...
    # FIX: Handle Ollama sending {} for empty arguments
...
async def mempalace_get_taxonomy():
    ...
    try:
        # Some versions put get_taxonomy in palace_graph, others in palace_logic
        try:
        ...
        return f"Taxonomy Tool unavailable in this library version: {str(e)}"

@mcp.tool()
async def mempalace_list_wings():
    """List all wings via graph stats."""
    from mempalace.mcp_server import _get_collection, _call_kg
    from mempalace.palace_graph import graph_stats # Ensure import is here
...
class MCPPathFixMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        if scope["type"] == "http":
            ...
            new_scope["root_path"] = ""
            if scope["path"] == "/sse/sse": new_scope["path"] = "/sse"
...
app = Starlette(); app.mount("/", router)
